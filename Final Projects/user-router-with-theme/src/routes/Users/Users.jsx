...
export default function Users() {
    ...
    const borderStyle = {
        border: `5px solid ${optionalStyle.borderColor}`,
    };
  ...
  return (
      <div style={{ display: "flex" }}>
          <div style={{ flex: "50% 1 0" }}>
              {users.map((user) => {
                  return (
                      <div key={user.id} className={`${mStyle.userStyle} ${selectedUser(user.id)}`} onClick={()=>{handleClickUser(user.id)}} style={borderStyle}>
                          <img src={user.image} alt={user.id} />
                          <h2 style={textStyles}>{user.name}</h2>
                      </div>
                  );
              })}
          </div>
